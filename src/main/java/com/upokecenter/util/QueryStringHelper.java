...
0xdc00));
    ...
    private static void ConvertLists(List<Object> dict) {
      for (int i = 0; i < dict.size(); ++i) {
        Map<String, Object> value = ((dict.get(i) is
              Map<String, Object>) ? (Map<String, Object>)dict.get(i) :
            null);
    ...
    private static void ConvertLists(Map<String, Object> dict) {
      for (Object key : new ArrayList<String>(dict.keySet())) {
        Map<String, Object> value = ((dict.get(key) is
              Map<String, Object>) ?
            (Map<String, Object>)dict.get(key) : null);
        // A list contains only indexes 0, 1, 2, and so on,
        // with no gaps.
        if (IsList (value)) {
          List<Object> newList = ConvertToList (value);
    ...
    public static Map<String, Object> QueryStringToDict(String query,
      ...
      for (Object keyvalue : ParseQueryString (query, delimiter)) {
        String[] path = GetKeyPath (keyvalue.get(0));
        ...
        for (int i = 0; i < path.length - 1; ++i) {
          if (!leaf.containsKey (path[i])) {
            ...
            Map<String, Object> o = ((leaf.get(path[i]) is
                  Map<String, Object>) ?
                (Map<String, Object>)leaf.get(path[i]) : null);
            if (o != null) {
              leaf = o;
            } else {
        ...
        if (leaf != null) {
          leaf.put(path[path.length - 1], keyvalue.get(1));
        }
      }
